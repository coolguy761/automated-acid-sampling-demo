/*
  Nextion M1 Half Retract -> M3 Slack -> M1 Finish - Uno + RTC
  -------------------------------------------------------------
  Purpose:
  - Run a focused Nextion-driven retract/slack test
  - Start from the extended/down position
  - Retract Motor 1 halfway
  - Run Motor 3 to pull slack out
  - Finish retracting Motor 1
  - Keep RTC clocks and exactly 2 short log lines
*/
...
const unsigned long STEP_M3_TILT_OUT_MS = 1800UL;
const unsigned long STEP_M3_TILT_HOME_MS = 1800UL;
...
const AutoStep VALIDATION_CYCLE[] = {
  ...
  { AXIS_M3, MOVE_FWD, STEP_M3_TILT_OUT_MS },
  { AXIS_M3, MOVE_REV, STEP_M3_TILT_HOME_MS }
};
...
void serviceMotion() {
  ...
  if (!stepHigh) {
    if ((unsigned long)(now - lastStepUs) >= (activeAxis == AXIS_M3 ? STEP_INTERVAL_M3_US : STEP_INTERVAL_M1_US)) {
...
void markStepComplete(uint8_t index) {
  if (index == 2) m1Done = true;
  if (index == 4) m3Done = true;
}
...
void copyStepLabel(uint8_t index, bool completeText, char *out, size_t outLen) {
  switch (index) {
    case 0: snprintf_P(out, outLen, completeText ? PSTR("S1DN") : PSTR("S1M1H")); break;
    case 1: snprintf_P(out, outLen, completeText ? PSTR("S2DN") : PSTR("S2M3SL")); break;
    case 2: snprintf_P(out, outLen, completeText ? PSTR("S3DN") : PSTR("S3M1F")); break;
    case 3: snprintf_P(out, outLen, completeText ? PSTR("S4DN") : PSTR("S4M3OT")); break;
    case 4: snprintf_P(out, outLen, completeText ? PSTR("S5DN") : PSTR("S5M3HM")); break;
...
void fillAxisStateText(char *out, size_t outLen) {
  ...
  else if (activeAxis == AXIS_M3) snprintf_P(out, outLen, activeDir == MOVE_FWD ? PSTR("M3 OUT") : PSTR("M3 SLACK"));
