...
void runM3ForLoop(Direction dir, unsigned long stepCount);
void serviceMotion();
...
void runM3ForLoop(Direction dir, unsigned long stepCount) {
  ...
  activeAxis = AXIS_M3;
  activeDir = dir;
  digitalWrite(dirPinFor(AXIS_M3), dirLevelFor(AXIS_M3, dir));
  digitalWrite(enablePinFor(AXIS_M3), enabledLevelFor(AXIS_M3));
  updateHmi();
...
void startAutoStep(uint8_t index) {
  ...
  appendLogRam(msgBuf);
  if (FULL_CYCLE[index].axis == AXIS_M3) {
    setDemoState(ST_AUTO_RUN);
    runM3ForLoop(FULL_CYCLE[index].dir, FULL_CYCLE[index].runMs);
    markStepComplete(index);
    copyStepLabel(index, true, msgBuf, sizeof(msgBuf));
    appendLogRam(msgBuf);
    setDemoState(ST_AUTO_WAIT);
  } else {
...
void startResetStep(uint8_t index) {
  if (resetCycle[index].axis == AXIS_M3) {
    setDemoState(ST_RESET_RUN);
    runM3ForLoop(resetCycle[index].dir, resetCycle[index].runMs);
    m3IsOut = false;
    appendLogP(PSTR("RSDN"));
    setDemoState(ST_RESET_WAIT);
  } else {
...
void handleCommand(char *cmd) {
  ...
  if (!strcmp(cmd, "M1F")) {
    ...
    m3IsOut = true;
    runM3ForLoop(MOVE_FWD, STEP_06_M3_TILT_OUT_STEPS);
  } else if (!strcmp(cmd, "M3R")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("M3HOM"));
    m3IsOut = false;
    runM3ForLoop(MOVE_REV, STEP_07_M3_TILT_HOME_STEPS);
...
void buildResetCycle() {
  ...
  if (m3IsOut) {
    ...
    resetCycle[resetStepCount].runMs = STEP_07_M3_TILT_HOME_STEPS;
    resetStepCount++;
  }
  if (m1IsDown) {
...
void captureActiveMotionState() {
  ...
  const bool mostlyFinished = plannedMs && (elapsedMs * 10UL >= plannedMs * 8UL);
...
unsigned long currentPlannedRunMs() {
  if (demoState == ST_AUTO_RUN && autoStepIndex < FULL_CYCLE_COUNT) {
    return FULL_CYCLE[autoStepIndex].runMs;
  }
  if (demoState == ST_RESET_RUN && resetStepIndex < resetStepCount) {
    return resetCycle[resetStepIndex].runMs;
  }
  return 0UL;
}
