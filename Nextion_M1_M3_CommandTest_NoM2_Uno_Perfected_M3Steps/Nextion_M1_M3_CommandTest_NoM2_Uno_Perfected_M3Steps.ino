...
const unsigned long STEP_04_M1_UP_MS          = 7850UL;
const unsigned long STEP_06_M3_TILT_OUT_STEPS = 2300UL;
const unsigned long STEP_07_M3_TILT_HOME_STEPS = 2300UL;
...
unsigned long motionProgress = 0;
...
void setupPins();
void stopAllMotion();
void startAxis(Axis axis, Direction dir, bool useSteps, unsigned long target);
void serviceMotion();
...
unsigned long currentPlannedRunMs();
...
void serviceMotion() {
  ...
  if (!stepHigh) {
    ...
    stepHigh = false;
    lastStepUs = now;
    if (motionUsesSteps && motionProgress < motionTarget) motionProgress++;
  }

  if (demoState == ST_IDLE && motionUsesSteps && motionTarget && motionProgress >= motionTarget) {
    stopAllMotion();
    updateHmi();
  }
}
...
void serviceSequence() {
  ...
  switch (demoState) {
    ...
    case ST_AUTO_RUN:
      if ((activeAxis == AXIS_M3 && motionUsesSteps)
            ? (motionTarget && motionProgress >= motionTarget)
            : (now - demoStateStartMs >= FULL_CYCLE[autoStepIndex].target)) {
        stopAllMotion();
    ...
    case ST_RESET_RUN:
      if ((activeAxis == AXIS_M3 && motionUsesSteps)
            ? (motionTarget && motionProgress >= motionTarget)
            : (now - demoStateStartMs >= resetCycle[resetStepIndex].target)) {
        stopAllMotion();
...
void startAutoStep(uint8_t index) {
  ...
  appendLogRam(msgBuf);
  startAxis(FULL_CYCLE[index].axis,
            FULL_CYCLE[index].dir,
            FULL_CYCLE[index].axis == AXIS_M3,
            FULL_CYCLE[index].target);
...
void startResetStep(uint8_t index) {
  startAxis(resetCycle[index].axis,
            resetCycle[index].dir,
            resetCycle[index].axis == AXIS_M3,
            resetCycle[index].target);
...
void handleCommand(char *cmd) {
  ...
  if (!strcmp(cmd, "M1F")) {
    ...
    m1IsDown = true;
    startAxis(AXIS_M1, MOVE_FWD, false, 0UL);
  } else if (!strcmp(cmd, "M1R")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("M1RET"));
    m1IsDown = false;
    startAxis(AXIS_M1, MOVE_REV, false, 0UL);
  } else if (!strcmp(cmd, "M3F")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("M3OUT"));
    m3IsOut = true;
    startAxis(AXIS_M3, MOVE_FWD, true, STEP_06_M3_TILT_OUT_STEPS);
  } else if (!strcmp(cmd, "M3R")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("M3HOM"));
    m3IsOut = false;
    startAxis(AXIS_M3, MOVE_REV, true, STEP_07_M3_TILT_HOME_STEPS);
...
void buildResetCycle() {
  ...
  if (m3IsOut) {
    ...
    resetCycle[resetStepCount].target = STEP_07_M3_TILT_HOME_STEPS;
    resetStepCount++;
  }
  if (m1IsDown) {
    resetCycle[resetStepCount].axis = AXIS_M1;
    resetCycle[resetStepCount].dir = MOVE_REV;
    resetCycle[resetStepCount].target = STEP_04_M1_UP_MS;
    resetStepCount++;
  }
}

void captureActiveMotionState() {
  const unsigned long plannedMs = currentPlannedRunMs();
  const unsigned long elapsedMs =
    (demoState == ST_AUTO_RUN || demoState == ST_RESET_RUN) ? (millis() - demoStateStartMs) : 0UL;
  const bool mostlyFinished = motionUsesSteps
    ? (motionTarget && (motionProgress * 10UL >= motionTarget * 8UL))
    : (plannedMs && (elapsedMs * 10UL >= plannedMs * 8UL));

  if (activeAxis == AXIS_M1) {
    if (activeDir == MOVE_FWD) m1IsDown = true;
    else if (activeDir == MOVE_REV) m1IsDown = !mostlyFinished;
  } else if (activeAxis == AXIS_M3) {
    if (activeDir == MOVE_FWD) m3IsOut = true;
    else if (activeDir == MOVE_REV) m3IsOut = !mostlyFinished;
  }
}

unsigned long currentPlannedRunMs() {
  if (demoState == ST_AUTO_RUN && autoStepIndex < FULL_CYCLE_COUNT && FULL_CYCLE[autoStepIndex].axis == AXIS_M1) {
    return FULL_CYCLE[autoStepIndex].target;
  }
  if (demoState == ST_RESET_RUN && resetStepIndex < resetStepCount && resetCycle[resetStepIndex].axis == AXIS_M1) {
    return resetCycle[resetStepIndex].target;
  }
  return 0UL;
}
