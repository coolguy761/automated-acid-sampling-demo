/*
  Nextion M1/M3 Retract-First Validation - Uno + 2 DRV8825 + RTC
  ---------------------------------------------------------------
  Start condition:
  - Mechanism begins extended

  START runs one cycle:
  1. M1 retract up
  2. M3 tilt out
  3. M3 tilt home
  4. M1 extend back down
*/
...
const unsigned long STEP_01_M1_UP_MS   = 8750UL;
const unsigned long STEP_02_M3_OUT_MS  = 1800UL;
const unsigned long STEP_03_M3_HOME_MS = 1800UL;
const unsigned long STEP_04_M1_DOWN_MS = 8750UL;
...
const AutoStep VALIDATION_CYCLE[] = {
  { AXIS_M1, MOVE_REV, STEP_01_M1_UP_MS },
  { AXIS_M3, MOVE_FWD, STEP_02_M3_OUT_MS },
  { AXIS_M3, MOVE_REV, STEP_03_M3_HOME_MS },
  { AXIS_M1, MOVE_FWD, STEP_04_M1_DOWN_MS }
};
...
void markStepComplete(uint8_t index) {
  ...
  else if (index == 1 || index == 2) m3Done = true;
}
...
void copyStepLabel(uint8_t index, bool completeText, char *out, size_t outLen) {
  switch (index) {
    case 0: snprintf_P(out, outLen, completeText ? PSTR("S1DN") : PSTR("S1M1UP")); break;
    case 1: snprintf_P(out, outLen, completeText ? PSTR("S2DN") : PSTR("S2M3OT")); break;
    case 2: snprintf_P(out, outLen, completeText ? PSTR("S3DN") : PSTR("S3M3HM")); break;
    case 3: snprintf_P(out, outLen, completeText ? PSTR("S4DN") : PSTR("S4M1DN")); break;
