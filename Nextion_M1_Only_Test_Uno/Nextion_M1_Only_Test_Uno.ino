/*
  Nextion M1 Only Test - Uno + RTC
  --------------------------------
  Purpose:
  - Test only Motor 1 from the existing Nextion screen
  - Keep RTC clock updates and lightweight status/log output
  - Provide one simple START cycle: extend down, then retract up
*/
...
const unsigned long STEP_01_M1_DOWN_MS    = 8350UL;
...
void serviceMotion() {
  ...
  if (!stepHigh) {
    if ((unsigned long)(now - lastStepUs) >= STEP_INTERVAL_M1_US) {
...
uint8_t enabledLevelFor(Axis axis) {
  return M1_ENA_ENABLED_LEVEL;
}

uint8_t disabledLevelFor(Axis axis) {
  return M1_ENA_DISABLED_LEVEL;
}

uint8_t dirLevelFor(Axis axis, Direction dir) {
  bool forwardUsesFwdLevel = M1_FORWARD_IS_DOWN;
...
void updateHmi() {
  ...
  snprintf(encBuf, sizeof(encBuf), "M1 TEST");
  snprintf(motorBuf, sizeof(motorBuf), "M1 ONLY");
  fillActiveText(activeAxis == AXIS_M1, "M1 ACTIVE:", motorFwdBuf, sizeof(motorFwdBuf));
  snprintf(motorRevBuf, sizeof(motorRevBuf), "STEP US: %lu", STEP_INTERVAL_M1_US);
