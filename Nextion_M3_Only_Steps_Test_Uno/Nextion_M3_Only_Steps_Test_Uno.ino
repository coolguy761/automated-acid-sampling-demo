...
const unsigned long STEP_01_M3_TILT_OUT_STEPS = 1800UL;
const unsigned long STEP_02_M3_TILT_HOME_STEPS = 1800UL;
...
const AutoStep TEST_CYCLE[] = {
  { AXIS_M3, MOVE_FWD, STEP_01_M3_TILT_OUT_STEPS },
  { AXIS_M3, MOVE_REV, STEP_02_M3_TILT_HOME_STEPS }
};
...
unsigned long targetStepCount = 0;
unsigned long completedStepCount = 0;
...
void startAutoStep(uint8_t index) {
  ...
  appendLogRam(msgBuf);
  startAxis(TEST_CYCLE[index].axis, TEST_CYCLE[index].dir, TEST_CYCLE[index].stepCount);
...
void handleCommand(char *cmd) {
  ...
  if (!strcmp(cmd, "M3F")) {
    ...
    m3IsOut = true;
    startAxis(AXIS_M3, MOVE_FWD, STEP_01_M3_TILT_OUT_STEPS);
    updateHmi();
  } else if (!strcmp(cmd, "M3R")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("M3HOM"));
    m3IsOut = false;
    startAxis(AXIS_M3, MOVE_REV, STEP_02_M3_TILT_HOME_STEPS);
    updateHmi();
...
void updateHmi() {
  ...
  snprintf(motorRevBuf, sizeof(motorRevBuf), "OUT STP: %lu", STEP_01_M3_TILT_OUT_STEPS);
