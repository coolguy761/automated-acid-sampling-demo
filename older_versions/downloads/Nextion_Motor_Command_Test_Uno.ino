/*
  Nextion Motor Command Test - Uno + 3 DRV8825 + RTC
  --------------------------------------------------
  Memory-optimized Uno build for tonight's bench tuning.
  - Full cycle sequencing
  - Manual motor commands from Nextion
  - RTC clock on all HMI pages
  - Timestamped event log on the log page
  - Direction flip flags and per-step timing constants at the top
*/
...
// Flip these three flags during bench tuning instead of changing logic below.
...
const AutoStep FULL_CYCLE[] = {
  ...
  { AXIS_M2, MOVE_FWD, STEP_02_M2_OPEN_MS },
  { AXIS_M2, MOVE_REV, STEP_03_M2_CLOSE_MS },
  { AXIS_M1, MOVE_REV, STEP_04_M1_UP_MS },
  { AXIS_M2, MOVE_FWD, STEP_05_M2_OPEN_AFTER_MS },
  { AXIS_M3, MOVE_FWD, STEP_06_M3_TILT_OUT_MS },
  { AXIS_M3, MOVE_REV, STEP_07_M3_TILT_HOME_MS },
  { AXIS_M2, MOVE_REV, STEP_08_M2_FINAL_CLOSE_MS }
};
...
void setup() {
  ...
  appendLogP(PSTR("FULL CYCLE READY"));
  updateHmi();
}
    ...
    case ST_AUTO_WAIT:
      if (now - demoStateStartMs >= SETTLE_MS) {
        ...
        if (autoStepIndex >= FULL_CYCLE_COUNT) {
          appendLogP(PSTR("FULL CYCLE COMPLETE"));
...
void beginAutoCycle() {
  autoStepIndex = 0;
  appendLogP(PSTR("START FULL CYCLE"));
  startAutoStep(autoStepIndex);
}
...
uint8_t stepPinFor(Axis axis) {
  switch (axis) {
    case AXIS_M1: return PIN_M1_STEP;
...
void handleCommand(char *cmd) {
  ...
  if (!strcmp(cmd, "M1F")) {
    ...
    setDemoState(ST_IDLE);
    appendLogP(PSTR("MANUAL M2 OPEN"));
    startAxis(AXIS_M2, MOVE_FWD);
  } else if (!strcmp(cmd, "M2R")) {
    setDemoState(ST_IDLE);
    appendLogP(PSTR("MANUAL M2 CLOSE"));
...
void pushClockText() {
  char clockBuf[15];
  if (rtcOnline) {
    snprintf(clockBuf, sizeof(clockBuf), "TIME: %s", currentTimeText);
  } else {
    strcpy(clockBuf, "TIME: RTC ERR");
  }
...
void copyStepLabel(uint8_t index, bool completeText, char *out, size_t outLen) {
  switch (index) {
    ...
    case 1: snprintf_P(out, outLen, completeText ? PSTR("STEP 2 COMPLETE") : PSTR("STEP 2 M2 OPEN")); break;
    case 2: snprintf_P(out, outLen, completeText ? PSTR("STEP 3 COMPLETE") : PSTR("STEP 3 M2 CLOSE")); break;
    case 3: snprintf_P(out, outLen, completeText ? PSTR("STEP 4 COMPLETE") : PSTR("STEP 4 M1 UP")); break;
    case 4: snprintf_P(out, outLen, completeText ? PSTR("STEP 5 COMPLETE") : PSTR("STEP 5 M2 OPEN")); break;
    case 5: snprintf_P(out, outLen, completeText ? PSTR("STEP 6 COMPLETE") : PSTR("STEP 6 M3 OUT")); break;
    case 6: snprintf_P(out, outLen, completeText ? PSTR("STEP 7 COMPLETE") : PSTR("STEP 7 M3 HOME")); break;
    case 7: snprintf_P(out, outLen, completeText ? PSTR("STEP 8 COMPLETE") : PSTR("STEP 8 M2 CLOSE")); break;
